...
UBTTask_OPMoveTo::UBTTask_OPMoveTo(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer)
{
	NodeName = "OP Move To";
	...
	// accept only objects and vectors
	BlackboardKey.AddObjectFilter(this, GET_MEMBER_NAME_CHECKED(UBTTask_OPMoveTo, BlackboardKey), UObject::StaticClass());
 	BlackboardKey.AddVectorFilter(this, GET_MEMBER_NAME_CHECKED(UBTTask_OPMoveTo, BlackboardKey));
}

EBTNodeResult::Type UBTTask_OPMoveTo::ExecuteTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory)
{
	...
	if (NodeResult == EBTNodeResult::InProgress && bObserveBlackboardValue)
	{
		...
		if (ensure(BlackboardComp))
		{
			...
			{
				UE_VLOG(MyController, LogBehaviorTree, Warning, TEXT("UBTTask_OPMoveTo::ExecuteTask \'%s\' Old BBObserverDelegateHandle is still valid! Removing old Observer."), *GetNodeName());
				BlackboardComp->UnregisterObserver(BlackboardKey.GetSelectedKeyID(), MyMemory->BBObserverDelegateHandle);
			}
			MyMemory->BBObserverDelegateHandle = BlackboardComp->RegisterObserver(BlackboardKey.GetSelectedKeyID(), this, FOnBlackboardChangeNotification::CreateUObject(this, &UBTTask_OPMoveTo::OnBlackboardValueChange));
		}
	}	
...
EBTNodeResult::Type UBTTask_OPMoveTo::PerformMoveTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory)
{
	...
	if (MyController && MyBlackboard)
	{
		...
		if (BlackboardKey.SelectedKeyType == UBlackboardKeyType_Object::StaticClass())
		{
			...
			{
				UE_VLOG(MyController, LogBehaviorTree, Warning, TEXT("UBTTask_OPMoveTo::ExecuteTask tried to go to actor while BB %s entry was empty"), *BlackboardKey.SelectedKeyName.ToString());
			}
		}
...
UAITask_MoveTo* UBTTask_OPMoveTo::PrepareMoveTask(UBehaviorTreeComponent& OwnerComp, UAITask_MoveTo* ExistingTask, FAIMoveRequest& MoveRequest)
{
...
EBlackboardNotificationResult UBTTask_OPMoveTo::OnBlackboardValueChange(const UBlackboardComponent& Blackboard, FBlackboard::FKey ChangedKeyID)
{
...
EBTNodeResult::Type UBTTask_OPMoveTo::AbortTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory)
{
	FBTMoveToTaskMemory* MyMemory = CastInstanceNodeMemory<FBTMoveToTaskMemory>(NodeMemory);
	if (!MyMemory->bWaitingForPath)
	{
...
void UBTTask_OPMoveTo::OnTaskFinished(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory, EBTNodeResult::Type TaskResult)
{
	FBTMoveToTaskMemory* MyMemory = CastInstanceNodeMemory<FBTMoveToTaskMemory>(NodeMemory);
	MyMemory->Task.Reset();
...
void UBTTask_OPMoveTo::TickTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory, float DeltaSeconds)
{
...
void UBTTask_OPMoveTo::OnMessage(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory, FName Message, int32 SenderID, bool bSuccess)
{
...
void UBTTask_OPMoveTo::OnGameplayTaskDeactivated(UGameplayTask& Task)
{
...
FString UBTTask_OPMoveTo::GetStaticDescription() const
{
...
void UBTTask_OPMoveTo::DescribeRuntimeValues(const UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory, EBTDescriptionVerbosity::Type Verbosity, TArray<FString>& Values) const
{
...
uint16 UBTTask_OPMoveTo::GetInstanceMemorySize() const
{
	return sizeof(FBTMoveToTaskMemory);
}

void UBTTask_OPMoveTo::PostLoad()
{
	Super::PostLoad();
...
FName UBTTask_OPMoveTo::GetNodeIconName() const
{
	return FName("BTEditor.Graph.BTNode.Task.MoveTo.Icon");
}

void UBTTask_OPMoveTo::OnNodeCreated()
{
	bStopOnOverlapNeedsUpdate = false;
}
